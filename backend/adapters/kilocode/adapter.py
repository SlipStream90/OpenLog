...
from backend.adapters.kilocode import event_mapper
...
class KiloCodeAdapter:
    ...
    def initialize(self) -> None:
        logger.debug("KiloCodeAdapter initialized")
    ...
    def capture_events(self, raw: dict[str, Any], source: EventSource = "hook") -> list[UniversalEvent]:
        ...
        try:
            ...
            logger.exception("Failed to normalize kilocode %s payload", source)
            return []
    ...
    def _ensure_session_id(self, event: UniversalEvent) -> UniversalEvent:
        ...
        logger.warning("KiloCode event %s had no session id; bucketing under %s", event.event.value, synthetic)
        return UniversalEvent(session_id=synthetic, timestamp=event.timestamp, agent=event.agent, event=event.event, model=event.model, file=event.file, metadata=event.metadata, source=event.source)
